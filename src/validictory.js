	...
	validictory.extend({
		...
		validation : {
			...
			init : function() {
				...
				$('input['+this.attributeName+'], textarea['+this.attributeName+'], select['+this.attributeName+']').each(function() {
					var field = $(this);
			...
			formSubmitHandler : function(e) {
				var self = arguments.callee.validation,
					form = $(this),
				...
				if (formInvalid) {
					//alert('Invalid fields! Submission not sent');
					e.preventDefault();
					return false;
				}
				else {
			...
			fieldFocusHandler : function() {
				...
				if (messageDialog && messageDialog.length>0) {
					//$('#validatorMessage_'+id).width($(this).width()).show().offset($(this).offset()); //.delay(50).fadeIn(arguments.callee.validation.messageFadeDuration);
					messageDialog.show();
			...
			fieldBlurHandler : function() {
				...
				if (id || id===0) {
					//$('#validatorMessage_'+id).fadeOut(arguments.callee.validation.messageFadeDuration);
			...
			validateField : function(field, validationConfig) {
				...
				for (i=0; i<config.validators.length; i++) {
					...
					if (response===false) {
						...
						if (!document.getElementById('validatorMessage_'+field.attr('data-validatorid'))) {
							//$('<div class="validatorMessage"><div class="inner">'+message+'</div></div>').insertBefore(field);
						...
						if (fieldErrorPlaceholder) {
							...
							//messageDialog.fadeIn(this.messageFadeDuration);
						}
						break;
					}
					if (response===true) {
				...
				//validatorConfig = value = config = i = validator = validatorCfg = response = hasFalse = hasTrue = pos = messageDialog = message = fieldLabel = null;
				return {
					valid		: !hasFalse,
					invalid		: hasFalse,
					hasTrue		: hasTrue,
			...
			fieldChangeHandler : function() {
				arguments.callee.validation.validateField(this);
			},
		...
		if (jsData) {
			for (i=jsData.attributes.length; i--; ) {
				...
				if (prop.substring(0,5)==='data-') {
					...
					cms.config[prop] = jsData.attributes[i].value;
				}
			}
		}
